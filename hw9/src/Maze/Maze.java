...
public class Maze {
  ...
  **/
    ...
    if(vert==1) {
    		...
    		int NumberOfWalls = horiz*(vert-1)+vert*(horiz-1);
    		Wall[] RandomWalls = new Wall[NumberOfWalls];
    		for (j = 0; j < vert - 1; j++) {
                for (i = 0; i < horiz; i++) {
                	RandomWalls[j*horiz+i] = new Wall(1,i,j);  
                }
              }
    		for (i = 0; i < horiz - 1; i++) {
                for (j = 0; j < vert; j++) {
                	RandomWalls[horiz*(vert-1)+i*vert+j] = new Wall(2,i,j);
                }
              }
    		Wall temp;
    		for(i = NumberOfWalls-1; i>0 ; i--) {
    			temp = RandomWalls[i];
    			j = randInt(i);
    			RandomWalls[i] = RandomWalls[j];
    			RandomWalls[j] = temp;
    		}
    		
    		DisjointSets set = new DisjointSets(horiz*vert);
    		for(i = 0;i<NumberOfWalls;i++) {
    			if(RandomWalls[i].type ==1) {//if the wall is horizontal, up element is (h,v) down element is (h,v+1)
    				if(set.find(RandomWalls[i].v*horiz+RandomWalls[i].h) != set.find((RandomWalls[i].v+1)*horiz+RandomWalls[i].h)) {
    					hWalls[RandomWalls[i].h][RandomWalls[i].v] = false;
    					set.union(set.find(RandomWalls[i].v*horiz+RandomWalls[i].h), set.find((RandomWalls[i].v+1)*horiz+RandomWalls[i].h));
    				}	
    			}else {//if the wall is vertical, left element is (h,v) right element is (h+1,v)
    				if(set.find(RandomWalls[i].v*horiz+RandomWalls[i].h) != set.find(RandomWalls[i].v*horiz+RandomWalls[i].h+1)) {
    					vWalls[RandomWalls[i].h][RandomWalls[i].v] = false;
    					set.union(set.find(RandomWalls[i].v*horiz+RandomWalls[i].h), set.find(RandomWalls[i].v*horiz+RandomWalls[i].h+1));
    				}
    			}
    		}
    }
